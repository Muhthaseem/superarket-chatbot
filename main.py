...
def get_response(user_input):
    ...
    best_match_tag = check_all_messages(split_message)

    # Check if the tag is 'shelf_locations' and handle it accordingly
    if best_match_tag == "shelf_locations":
        for intent in intents['intents']:
    ...
    # Normal response for other tags
    for intent in intents['intents']:
...
def send_message(event=None):
    ...
    # Set the appropriate light based on the response
    if response == "I'm sorry, I didn't understand that.":
        light_red.config(bg='red')
        light_blue.config(bg='gray')
        light_green.config(bg='gray')
    else:
        light_green.config(bg='gray')
        light_blue.config(bg='blue')
        light_red.config(bg='gray')

def user_typing(event=None):
    light_green.config(bg='green')
    light_blue.config(bg='gray')
    light_red.config(bg='gray')
...
light_frame = tk.Frame(root, bg='#2e2e2e')
light_frame.pack(pady=(10, 0))

light_red = tk.Label(light_frame, width=2, height=1, bg='gray')
light_red.grid(row=0, column=0, padx=5)
light_blue = tk.Label(light_frame, width=2, height=1, bg='gray')
light_blue.grid(row=0, column=1, padx=5)
light_green = tk.Label(light_frame, width=2, height=1, bg='gray')
light_green.grid(row=0, column=2, padx=5)
...
# Detect user typing
user_input_entry.bind("<Key>", user_typing)
